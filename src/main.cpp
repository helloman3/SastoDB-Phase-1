...
struct Column {
    ...
    bool isPK = false, isUnique = false, isNotNull = false;
...
namespace Utils {
    ...
    inline string toUpper(string s) { for (char& c : s) c = (char)toupper(c); return s; }
    ...
    inline vector<string> split(const string& text, char del) {
        vector<string> parts; string cur = ""; bool inQ = false;
        for (char c : text) {
            if (c == '\'' || c == '"') inQ = !inQ;
            if (c == del && !inQ) { parts.push_back(trim(cur)); cur.clear(); }
            else cur += c;
        }
        parts.push_back(trim(cur));
        return parts;
    }
    inline bool getParen(const string& s, string& inside) {
        size_t a = s.find('('), b = s.rfind(')');
        if (a == string::npos || b == string::npos || b <= a) return false;
        inside = s.substr(a + 1, b - a - 1);
        return true;
    }
}
...
private:
    ...
    bool matchCondition(const string& val, const string& type, const string& op, const string& target) const {
        string clean = Utils::stripQuotes(target);
        if (type == "INT" || type == "INTEGER") {
            int a = 0, b = 0;
            try { a = stoi(Utils::trim(val)); b = stoi(clean); } catch (...) { return false; }
            return (op=="="||op=="==")?a==b:(op=="!=")?a!=b:(op=="<")?a<b:(op==">")?a>b:(op=="<=")?a<=b:(op==">=")?a>=b:false;
        } else if (type == "FLOAT" || type == "DOUBLE") {
            double a = 0, b = 0;
            try { a = stod(Utils::trim(val)); b = stod(clean); } catch (...) { return false; }
            return (op=="="||op=="==")?a==b:(op=="!=")?a!=b:(op=="<")?a<b:(op==">")?a>b:(op=="<=")?a<=b:(op==">=")?a>=b:false;
        }
        return (op=="="||op=="==")?val==clean:(op=="!=")?val!=clean:(op=="<")?val<clean:(op==">")?val>clean:false;
    }
    void printBorder(const vector<int>& w) const {
        cout << "+"; for (int width : w) cout << string(width + 2, '-') << "+"; cout << "\n";
    }
...
public:
    ...
    bool insertRow(const Row& row, string& err) {
        if (row.size() != columns.size()) {
            err = "Column count mismatch: Expected " + to_string(columns.size()) + " values, received " + to_string(row.size());
            return false;
        }
        for (size_t i = 0; i < columns.size(); i++) {
            string val = row[i];
            bool isNull = (val.empty() || Utils::toUpper(val) == "NULL");
            if (columns[i].isNotNull && isNull) { err = "Column '" + columns[i].name + "' cannot be NULL."; return false; }
            if (!isNull && columns[i].isInt() && !Utils::isNumeric(val)) { err = "Invalid integer value '" + val + "' for column '" + columns[i].name + "'."; return false; }
            if (!isNull && (columns[i].isPK || columns[i].isUnique)) {
                for (const auto& r : rows) {
                    if (r[i] == val) {
                        err = "Duplicate value '" + val + "' violates " + (columns[i].isPK ? "PRIMARY KEY" : "UNIQUE") + " constraint on column '" + columns[i].name + "'.";
                        return false;
                    }
                }
            }
    ...
    bool selectRows(const string& whereCol, const string& op, const string& whereVal, int& count) const {
        int wIdx = -1;
        if (!whereCol.empty() && (wIdx = getColumnIndex(whereCol)) == -1) {
        ...
        for (size_t r = 0; r < rows.size(); r++) {
            if (wIdx < 0 || matchCondition(rows[r][wIdx], columns[wIdx].type, op, whereVal)) {
                matched.push_back(r);
                for (size_t i = 0; i < columns.size(); i++) widths[i] = max(widths[i], (int)rows[r][i].length());
            }
        }
        count = matched.size();
        printBorder(widths);
        cout << "|"; for (size_t i = 0; i < columns.size(); i++) cout << " " << left << setw(widths[i]) << columns[i].name << " |";
        cout << "\n"; printBorder(widths);
        for (int rIdx : matched) {
            cout << "|"; for (size_t i = 0; i < columns.size(); i++) cout << " " << left << setw(widths[i]) << rows[rIdx][i] << " |";
            cout << "\n";
        }
        printBorder(widths);
        return true;
    ...
    bool updateRows(const string& targetCol, const string& newVal, const string& whereCol, const string& op, const string& whereVal, int& updated, string& err) {
        updated = 0;
        int targetIdx = getColumnIndex(targetCol);
        if (targetIdx == -1) { err = "Target column '" + targetCol + "' not found."; return false; }
        int wIdx = -1;
        if (!whereCol.empty() && (wIdx = getColumnIndex(whereCol)) == -1) { err = "WHERE column '" + whereCol + "' not found."; return false; }
        ...
        if (!isNull && columns[targetIdx].isInt() && !Utils::isNumeric(cleanVal)) { err = "Invalid integer value '" + cleanVal + "' for column '" + columns[targetIdx].name + "'."; return false; }
        ...
        if (!isNull && (columns[targetIdx].isPK || columns[targetIdx].isUnique)) {
            ...
            if (matched.size() > 1) { err = "Cannot update multiple rows to identical value under " + cType + " constraint."; return false; }
            for (size_t r = 0; r < rows.size(); r++) {
                bool isMatch = false; for (int m : matched) if (m == (int)r) isMatch = true;
                if (!isMatch && rows[r][targetIdx] == cleanVal) { err = "Duplicate value '" + cleanVal + "' violates " + cType + " constraint."; return false; }
            }
        }
        for (int r : matched) { rows[r][targetIdx] = cleanVal; updated++; }
        return true;
    }
    ...
    bool deleteRows(const string& whereCol, const string& op, const string& whereVal, int& deleted) {
        ...
        vector<Row> kept;
        for (const auto& r : rows) {
            if (wIdx >= 0 && matchCondition(r[wIdx], columns[wIdx].type, op, whereVal)) deleted++;
            else if (wIdx < 0) deleted++;
    ...
    void displaySchema() const {
        ...
        printBorder(w);
        cout << "| " << left << setw(w[0]) << "Column Name" << " | " << setw(w[1]) << "Type" << " | " << setw(w[2]) << "Primary Key" << " | " << setw(w[3]) << "Unique" << " | " << setw(w[4]) << "Not Null" << " |\n";
        printBorder(w);
        for (const auto& c : columns) {
            cout << "| " << left << setw(w[0]) << c.name << " | " << setw(w[1]) << c.type << " | " << setw(w[2]) << (c.isPK ? "YES" : "NO") << " | " << setw(w[3]) << (c.isUnique ? "YES" : "NO") << " | " << setw(w[4]) << (c.isNotNull ? "YES" : "NO") << " |\n";
        }
        printBorder(w);
    }

    bool saveToFile(const string& file) const {
        ofstream out(file); if (!out.is_open()) return false;
        out << "TABLE:" << name << "\nCOLUMNS:" << columns.size() << "\n";
        for (const auto& c : columns) out << c.name << ":" << c.type << ":" << (c.isPK?1:0) << ":" << (c.isUnique?1:0) << ":" << (c.isNotNull?1:0) << "\n";
        out << "ROWS:" << rows.size() << "\n";
        for (const auto& r : rows) {
            for (size_t c = 0; c < r.size(); c++) out << r[c] << (c + 1 < r.size() ? "\t" : "");
            out << "\n";
        }
        return true;
    }
    ...
    bool loadFromFile(const string& file) {
        ifstream in(file); if (!in.is_open()) return false;
        columns.clear(); rows.clear(); string line;
        ...
        for (int i = 0; i < numCols; i++) {
            ...
            if (p.size() >= 5) addColumn({p[0], p[1], p[2]=="1", p[3]=="1", p[4]=="1"});
        }
...
private:
    string dbName;
    vector<Table> tables;
    string getFile(const string& t) const { return t + "_data.txt"; }
public:
    Database(string name = "student_db") : dbName(name) { loadAll(); }
    ~Database() { saveAll(); }
    Table* getTable(const string& name) {
        for (auto& t : tables) if (Utils::toUpper(t.getName()) == Utils::toUpper(name)) return &t;
        return nullptr;
    }
    ...
    void listTables() const {
        ...
        for (const auto& t : tables)
            cout << "| " << left << setw(20) << t.getName() << " | " << setw(10) << t.getColumnCount() << " | " << setw(10) << t.getRowCount() << " |\n";
        cout << "+----------------------+------------+------------+\n" << tables.size() << " table(s) in total.\n";
    }
    void saveAll() {
        ofstream cat("db_catalog.txt"); if (!cat.is_open()) return;
        for (const auto& t : tables) { cat << t.getName() << "\n"; t.saveToFile(getFile(t.getName())); }
    }
    void loadAll() {
        ifstream cat("db_catalog.txt"); if (!cat.is_open()) return;
        tables.clear(); string tName;
        while (getline(cat, tName)) {
            tName = Utils::trim(tName);
            if (tName.empty()) continue;
            Table t(tName); if (t.loadFromFile(getFile(tName))) tables.push_back(t);
        }
    }
};

// --- 5. QUERY PARSER & DISPATCHER ---
bool parseWhere(string wherePart, string& col, string& op, string& val) {
    wherePart = Utils::trim(wherePart);
    for (const char* o : {"!=", "<=", ">=", "=="}) {
        size_t p = wherePart.find(o);
        if (p != string::npos) {
            col = Utils::trim(wherePart.substr(0, p)); op = o;
...
void executeSQL(Database& db, string sql) {
    sql = Utils::trim(sql); if (sql.empty()) return;
    ...
    if (upper.rfind("VIEW", 0) == 0 || upper.rfind("SHOW", 0) == 0) {
        ...
        if (target.empty() || Utils::toUpper(target) == "TABLES") db.listTables();
        else { Table* t = db.getTable(target); if (!t) cout << "Error: Table '" << target << "' not found.\n"; else t->displaySchema(); }
        return;
    }
    if (upper.rfind("CREATE TABLE", 0) == 0) {
        size_t pPos = sql.find('(');
        if (pPos == string::npos) { cout << "Error: Missing '(' in CREATE TABLE.\n"; return; }
        string tName = Utils::trim(sql.substr(12, pPos - 12)), inside;
        if (tName.empty()) { cout << "Error: Table name cannot be empty in CREATE TABLE.\n"; return; }
        if (!Utils::getParen(sql, inside)) { cout << "Error: Invalid parentheses in CREATE TABLE.\n"; return; }
        vector<Column> cols; int pkCount = 0;
        for (const auto& def : Utils::split(inside, ',')) {
            auto words = Utils::split(def, ' '); if (words.empty() || words[0].empty()) continue;
            Column col; col.name = words[0]; col.type = (words.size() > 1) ? Utils::toUpper(words[1]) : "TEXT";
            for (size_t w = 1; w < words.size(); w++) {
                string uw = Utils::toUpper(words[w]);
                if (uw == "PRIMARY" && w + 1 < words.size() && Utils::toUpper(words[w + 1]) == "KEY") { col.isPK = col.isUnique = col.isNotNull = true; pkCount++; w++; }
                else if (uw == "UNIQUE") col.isUnique = true;
                else if (uw == "NOT" && w + 1 < words.size() && Utils::toUpper(words[w + 1]) == "NULL") { col.isNotNull = true; w++; }
            }
            cols.push_back(col);
        }
        if (pkCount > 1) { cout << "Error: Table cannot have multiple PRIMARY KEY columns.\n"; return; }
        string err;
        if (db.createTable(tName, cols, err)) cout << "Table '" << tName << "' created successfully.\n";
        else cout << "Error: " << err << "\n";
        return;
    }
    if (upper.rfind("DROP TABLE", 0) == 0) {
        string tName = Utils::trim(sql.substr(10)), err;
    ...
    if (upper.rfind("INSERT INTO", 0) == 0) {
        ...
        string tName = Utils::trim(sql.substr(11, vPos - 11)), inside, err;
        Table* t = db.getTable(tName);
        if (!t) { cout << "Error: Table '" << tName << "' not found.\n"; return; }
        if (!Utils::getParen(sql.substr(vPos), inside)) { cout << "Error: Missing '()' after VALUES.\n"; return; }
        Row r; for (const auto& tok : Utils::split(inside, ',')) r.push_back(Utils::stripQuotes(tok));
        if (t->insertRow(r, err)) { db.saveAll(); cout << "1 row inserted successfully.\n"; }
        else cout << "Error: " << err << "\n";
        return;
    }
    if (upper.rfind("SELECT", 0) == 0) {
        ...
        string rest = Utils::trim(sql.substr(fPos + 6)), tName = rest, wCol = "", op = "", wVal = "";
        size_t wPos = Utils::toUpper(rest).find(" WHERE ");
        if (wPos != string::npos) {
            tName = Utils::trim(rest.substr(0, wPos));
            if (!parseWhere(rest.substr(wPos + 7), wCol, op, wVal)) { cout << "Error: Invalid WHERE clause syntax in SELECT.\n"; return; }
        }
        Table* t = db.getTable(tName);
        if (!t) { cout << "Error: Table '" << tName << "' not found.\n"; return; }
        int count = 0; t->selectRows(wCol, op, wVal, count); cout << count << " row(s) returned.\n";
        return;
    }
    if (upper.rfind("UPDATE", 0) == 0) {
        size_t sPos = upper.find(" SET ");
        if (sPos == string::npos) { cout << "Error: Missing 'SET' in UPDATE.\n"; return; }
        string tName = Utils::trim(sql.substr(6, sPos - 6)), rest = Utils::trim(sql.substr(sPos + 5));
        Table* t = db.getTable(tName);
        if (!t) { cout << "Error: Table '" << tName << "' not found.\n"; return; }
        size_t wPos = Utils::toUpper(rest).find(" WHERE ");
        string setPart = rest, wCol = "", op = "", wVal = "", err;
        if (wPos != string::npos) {
            setPart = Utils::trim(rest.substr(0, wPos));
            if (!parseWhere(rest.substr(wPos + 7), wCol, op, wVal)) { cout << "Error: Invalid WHERE clause syntax in UPDATE.\n"; return; }
        }
        size_t eqPos = setPart.find('=');
        if (eqPos == string::npos) { cout << "Error: Invalid SET expression: expected 'col = val'.\n"; return; }
        int updated = 0;
        if (t->updateRows(Utils::trim(setPart.substr(0, eqPos)), Utils::trim(setPart.substr(eqPos + 1)), wCol, op, wVal, updated, err)) {
            db.saveAll(); cout << updated << " row(s) updated successfully.\n";
        } else cout << "Error: " << err << "\n";
        return;
    }
    if (upper.rfind("DELETE FROM", 0) == 0) {
        string rest = Utils::trim(sql.substr(11)), tName = rest, wCol = "", op = "", wVal = "";
        size_t wPos = Utils::toUpper(rest).find(" WHERE ");
        if (wPos != string::npos) {
            tName = Utils::trim(rest.substr(0, wPos));
            if (!parseWhere(rest.substr(wPos + 7), wCol, op, wVal)) { cout << "Error: Invalid WHERE clause syntax in DELETE.\n"; return; }
        }
        Table* t = db.getTable(tName);
        if (!t) { cout << "Error: Table '" << tName << "' not found.\n"; return; }
        int deleted = 0;
        if (t->deleteRows(wCol, op, wVal, deleted)) { db.saveAll(); cout << deleted << " row(s) deleted successfully.\n"; }
        return;
    }
...
int main() {
    ...
    while (true) {
        ...
        size_t cPos = line.find("--"); if (cPos != string::npos) line = line.substr(0, cPos);
        string trimmed = Utils::trim(line); if (trimmed.empty()) continue;
        ...
        if (fullQuery.empty() && trimmed[0] == '.') fullQuery = trimmed;
        else {
